...
const columns: ColumnConfig[] = [
  { accessorKey: "name", header: "שם מקור הכנסה", type: "text" },
];
...
export default function IncomeSourcesTab() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["settings-income-sources"],
    queryFn: () => getAllIncomeSources(),
  });

  const deleteMutation = useMutation({
    mutationFn: deleteIncomeSourceById,
    onSuccess: () => {
      refetch();
    },
  });
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      const promise = upsertIncomeSource({
        id: editingItem?.id,
        name: value.name,
      });
    ...
    <>
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? "עריכת מקור הכנסה" : "הוספת מקור הכנסה"}
            </DialogTitle>
          </DialogHeader>
          <form
            onSubmit={(e) => {
          ...
          >
            <form.AppField
              name="name"
              children={(f) => <f.TextField label="שם מקור הכנסה" />}
            />
